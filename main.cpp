...
void DrawAntiAliasingLine(axGC* gc, const axPoint& point1, const axPoint& point2, const int& width, axColor color)
{
	...
	int wx = abs(dx) + cos_theta, wy = abs(dy) + sin_theta;
	...
	gc->DrawTexture(texture, axRect(100, 100, wx, wy));
}
...
void Desktop::OnPaint()
{
	...
	gc->DrawImageResize(_wallPaper, axPoint(0, 0), rect.size, 1.0);
	...
	DrawAntiAliasingLine(gc, axPoint(100, 100), axPoint(200, 500), 3, color);
