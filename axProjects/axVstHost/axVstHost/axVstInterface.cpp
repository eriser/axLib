                                       ...
                                       void *ptr, float opt)
    {
        switch (opcode)
        {
            ...
            case audioMasterIdle:
                axPrint("Plugin requested value of opcode : audioMasterIdle");
                effect->dispatcher(effect, effEditIdle, 0, 0, 0, 0);
                // Handle other opcodes here... there will be lots of them
            case audioMasterGetTime:
                axPrint("Plugin requested value of opcode : audioMasterGetTime");
            ...
            default:
                axPrint("Plugin requested value of opcode : ", opcode);
                return NULL;
        }
    }
}
...
void axVstInterface::LoadVstPlugin(const std::string& vst_path)
{
    ...
    axModuleLoader<AEffect*, VstIntPtr> loader(vst_path, "VSTPluginMain");
    std::function<AEffect*(VstIntPtr)> vst_entry = loader.GetHandle();
    
    _plugin = vst_entry((VstIntPtr)audioMasterCallback(hostCallback));
    
    if(!_plugin)
    {
        axError("Can't load vst plugin.");
    }
    //_plugin = loader.GetFunctionHandle()();
...
int axVstInterface::configurePluginCallbacks() 
{
	...
	{
		axPrint("Plugin's magic number is bad.");
		return -1;
	}
