...
//    _filter->SetGain(1.0);
//    
//    _freqModEnvAmount = 0.5;
//    _filter->SetFreqEnvelopeAmountPtr(&_freqModEnvAmount);
//    
...
//{
//    _filter->SetFreq(axClamp<double>(freq, 30.0, 5000.0));
//}
//
...
//                {
//                    // Attack.
//                    ampEnvValue = axClamp<double>(_decayIndex / 20.0, 0.0, 1.0);
//                }
//                else
//                {
//                    // Decay.
//                    ampEnvValue = 1.0 - ((_decayIndex) / decaySample);
//                    ampEnvValue = axClamp<double>(ampEnvValue, 0.0, 1.0);
//                }
//                
//            }
//            
//            axFloat filterFreqModEnvValue = ampEnvValue;
//            _filter->SetFreqEnvelopePtr(&filterFreqModEnvValue);
//            
//            
...
//            //            *output++ = *output * v * _volume;
//            
//            double totalVolume = ampEnvValue * _volume;
//            *output++ = filter_processed.l * totalVolume;
//            *output++ = filter_processed.r * totalVolume;
//        }
//    }
//    else
//    {
...
axPanel(parent, rect)
{
    std::string app_path = std::string("/Users/alexarse/Project/axLib/axProjects/axTB303Vst/build/UninstalledProducts/resources/");
...
axPanel(parent, rect)
{
    std::cout << "ENTER PROJECT" << std::endl;
    std::string app_path = std::string("/Users/alexarse/Project/axLib/axProjects/axTB303Vst/build/UninstalledProducts/resources/");//axApp::GetInstance()->GetAppDirectory();
    ...
    std::cout << "ENTER Knobs" << std::endl;
    axKnobInfo knob_info(axColor(0.3, 0.3, 0.3, 0.0),
                         axColor(0.3, 0.3, 0.3, 0.0),
                         axColor(0.3, 0.3, 0.3, 0.0),
                         128,
                         axSize(50, 50),
                         app_path + std::string("axKnobTB303_50x50.png"),
                         app_path + std::string("axKnobTB303_50x50.png"));
    
    axKnob::axKnobBuilder knobBuilder(this, axSize(50, 50), knob_info, 0, 10);
    
    axKnob* speed = knobBuilder.Create(axPoint(15, 25));
    axKnob* volume = knobBuilder.Create(GetOnVolumeChange());
    axKnob* tuning = knobBuilder.Create(axPoint(239, 25), GetOnTuningChange());
    axKnob* freq = knobBuilder.Create(GetOnFreqChange());
    axKnob* res = knobBuilder.Create(GetOnResChange());
    axKnob* envAmnt = knobBuilder.Create(GetOnEnvAmountChange());
    axKnob* decay = knobBuilder.Create(GetOnDecayChange());
    axKnob* accent = knobBuilder.Create();
    ...
    axButton* prefBtn = new axButton(this,
                                     ...
                                     app_path + std::string("settings.png"),
                                     "",
                                     axBUTTON_SINGLE_IMG);
...
void MyProject::OnEnvAmountChange(const axKnobMsg& msg)
{
    std::cout << "Env amount : " << msg.GetValue() << std::endl;
//    MyAudioSynth::GetInstance()->SetFreqModEnvAmount(msg.GetValue());
}
